...
'''def get_route_fly(pickup_lon, pickup_lat, dropoff_lon, dropoff_lat):
    loc = f"{pickup_lon},{pickup_lat};{dropoff_lon},{dropoff_lat}"
    url = "http://router.project-osrm.org/route/v1/driving/"
    logger.info(f"My URL - {url}{pickup_lon},{pickup_lat};{dropoff_lon},{dropoff_lat}")
    r = requests.get(url + loc)
    if r.status_code!= 200:
        return {}
    res = r.json()
    logger.info(f"My routes - {res['routes'][0]['geometry']}")
    logger.info(f"My start_point - {res['waypoints'][0]['location'][1], res['waypoints'][0]['location'][0]}")
    logger.info(f"My end_point - {res['waypoints'][1]['location'][1], res['waypoints'][1]['location'][0]}")
...
def get_route_fly(pickup_lon, pickup_lat, air_lat_1, air_long_1, air_lat_2, air_long_2, dropoff_lon, dropoff_lat):
    ...
    url = "http://router.project-osrm.org/route/v1/swiming/"
    logger.info(f"My URL - {url}{pickup_lon},{pickup_lat};{air_lat_1},{air_long_1};{air_lat_2},{air_long_2};{dropoff_lon},{dropoff_lat}")
    r = requests.get(url + loc)
    if r.status_code!= 200:
        return {}
    res = r.json()
